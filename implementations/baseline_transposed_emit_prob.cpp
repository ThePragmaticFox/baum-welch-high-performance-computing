...
static size_t comp_bw_trans(const BWdata& bw);


REGISTER_FUNCTION_TRANSPOSE_EMIT_PROB(comp_bw_trans, "transposed", "TEST: Baseline but transposed emit_prob");


size_t comp_bw_trans(const BWdata& bw){
...
inline void forward_step(const BWdata& bw) {
    for (size_t k = 0; k < bw.K; k++) {
        ...
        for (size_t n = 0; n < bw.N; n++) {
            bw.alpha[(k*bw.T + 0)*bw.N + n] = bw.init_prob[n]*bw.emit_prob[bw.observations[k*bw.T + 0]*bw.N + n];
        ...
        for (size_t t = 1; t < bw.T; t++) {
            ...
            for (size_t n0 = 0; n0 < bw.N; n0++) {
                ...
                bw.alpha[(k*bw.T + t)*bw.N + n0] = bw.emit_prob[bw.observations[k*bw.T + t]*bw.N + n0] * alpha_temp;
    ...
    for (size_t k = 0; k < bw.K; k++) {
        ...
        for (int t = bw.T-2; t >= 0; t--) {
            for (size_t n0 = 0; n0 < bw.N; n0++) {
                ...
                for (size_t n1 = 0; n1 < bw.N; n1++) {
                    beta_temp += bw.beta[(k*bw.T + (t+1))*bw.N + n1] * bw.trans_prob[n0*bw.N + n1] * bw.emit_prob[bw.observations[k*bw.T + (t+1)]*bw.N + n1];
                }
            ...
            for (size_t n0 = 0; n0 < bw.N; n0++) {
                for (size_t n1 = 0; n1 < bw.N; n1++) {
                    bw.sigma[((k*bw.T + t)*bw.N + n0)*bw.N + n1] = \
                        bw.alpha[(k*bw.T + t)*bw.N + n0]*bw.trans_prob[n0*bw.N + n1]*bw.beta[(k*bw.T + (t+1))*bw.N + n1]*bw.emit_prob[bw.observations[k*bw.T + (t+1)]*bw.N + n1];
                }
            }
        }
...
inline void update_emit_prob(const BWdata& bw) {
    ...
    for (size_t n = 0; n < bw.N; n++) {
        for (size_t m = 0; m < bw.M; m++) {
            ...
            bw.emit_prob[m*bw.N + n] = numerator_sum / denominator_sum;
        }
    }
}
